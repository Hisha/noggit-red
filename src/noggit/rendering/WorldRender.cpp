...
)
{
  ...
  if (!render_settings.minimap_render)
  {
    ...
    bool render_local_lightning = render_settings.current_editing_mode == editing_mode::light ? true : local_lightning;
  ...
  if(!render_settings.minimap_render && render_settings.current_display_mode == display_mode::in_3D && render_settings.draw_sky)
  {
  ...
  if (!_world->mapIndex.hasAGlobalWMO() && render_settings.draw_fog && render_settings.draw_terrain)
  {
    ...
    _horizon_render->draw (model_view, projection, 
      ...
      frustum,
      camera_pos,
      render_settings.current_display_mode);
  }
...

    {
      ...
      for (auto const& pair : _world->_loaded_tiles_buffer)
      {
        ...
        tile->renderer()->draw(
            ...
            , render_settings.current_editing_mode == editing_mode::minimap
  ...
  if (render_settings.current_editing_mode == editing_mode::object && _world->has_multiple_model_selected())
  {
  ...
  if (render_settings.current_editing_mode == editing_mode::ground && render_settings.ground_editing_brush == eTerrainType_Vertex)
  {
    ...
    for (auto& pair : tile->getObjectInstances())
    {
      ...
      if (pair.second[0]->which() == eMODEL)
      {
        ...
        for (auto& instance : pair.second)
        {
          ...
          if (!render && m2_instance->isInRenderDist(_cull_distance, camera_pos, render_settings.current_display_mode)
            ...
            if (render_settings.draw_wmo_doodads)
            {
              ...
              for (auto& pair : *doodads)
              {
                for (auto& doodad : pair.second)
                {
                    ...
                    if (!doodad.isInRenderDist(_cull_distance, camera_pos, render_settings.current_display_mode))
                      continue;
        ...
        for (auto const& pair : models_to_draw)
        {
          ...
          /*if (draw_hidden_models || !pair.first->is_hidden())*/ // now done when building models_to_draw
          {
            pair.first->renderer()->draw( model_view
                ...
                , false
                , render_settings.draw_model_animations
                , render_settings.current_editing_mode == editing_mode::object
                , draw_animated_boxes
            );
          ...
          if (/*render_settings.current_editing_mode == editing_mode::object*/
  ...
  if (render_settings.angled_mode || render_settings.use_ref_pos)
  {
    ...
    if (render_settings.angled_mode && render_settings.current_editing_mode == editing_mode::flatten_blur)
    {
  ...
  if (render_settings.current_editing_mode == editing_mode::light && render_settings.alpha_light_sphere > 0.0f)
  {
...
)
{
  ...
  renderParams.current_editing_mode = editing_mode::minimap;
